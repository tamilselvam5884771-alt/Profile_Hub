...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Connect to AuthContext login flow
      await login({ email: formData.email, password: formData.password });
      navigate('/dashboard', { replace: true });
    } catch (err) {
      // Handle and display backend validation/credential errors
      const errorMessage =
  ...
  // Show a full page loader when restoring user session on first load
  if (authLoading) {
    return (
      <div className="min-h-screen w-full bg-gradient-to-br from-[#021d1e] via-[#042c2e] to-[#073630] flex items-center justify-center p-4">
        <div className="text-amber-400 font-mono tracking-widest text-lg animate-pulse">
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-[#021d1e] via-[#042c2e] to-[#073630] flex items-center justify-center p-4 md:p-12 relative overflow-hidden font-sans selection:bg-amber-500 selection:text-black">
      ...
      {/* Background Decorative Tech Lines / Glow Effect */}
      ...
      {/* Main Container Split exactly like the system display */}
      <div className="w-full max-w-7xl grid md:grid-cols-2 gap-4 lg:gap-12 items-center z-10">
        
        {/* LEFT SIDE: Logo, Side Network Tree & Bottom Quote */}
        <motion.div 
        ...
        >
          {/* Main Logo and Graphic Layout Group */}
          <div className="flex flex-row items-center justify-between w-full relative mt-8">
            ...
            <div className="flex flex-col items-start space-y-4 z-10">
              <div className="relative w-36 h-36 flex items-center justify-center text-amber-400 drop-shadow-[0_4px_12px_rgba(212,175,55,0.2)]">
                ...
                <User className="w-14 h-14 stroke-[1.2]" />
              </div>
          ...
          {/* Cursive Quote at the Bottom Left */}
        ...
        {/* RIGHT SIDE: Premium Glassmorphic Login Box */}
        <motion.div
          ...
          <form onSubmit={handleSubmit} className="space-y-6 relative">
            ...
            {error && (
              <motion.div 
                ...
                className="p-3.5 bg-red-950/40 border border-red-500/30 rounded-xl text-red-200 text-xs tracking-wide shadow-md"
              >
                {error}
              </motion.div>
            )}
            ...
            <div className="space-y-1.5">
              <label className="text-xs uppercase tracking-widest text-amber-200/60 font-medium pl-1">
                Email Address
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-teal-950/60 border border-teal-800/60 rounded-xl text-amber-100 placeholder-teal-400/20 focus:outline-none focus:border-amber-400/80 focus:ring-1 focus:ring-amber-400/20 transition-all duration-300 text-sm"
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-3 bg-teal-950/60 border border-teal-800/60 rounded-xl text-amber-100 placeholder-teal-400/20 focus:outline-none focus:border-amber-400/80 focus:ring-1 focus:ring-amber-400/20 transition-all duration-300 text-sm"
                />
                <button
                  type="button"
            ...
            <div className="flex items-center justify-between text-xs pt-0.5">
              <label className="flex items-center gap-2 text-teal-300/60 cursor-pointer select-none">
                <input
                  type="checkbox"
                  name="rememberMe"
                  checked={formData.rememberMe}
                  onChange={handleChange}
                  className="accent-amber-400 rounded bg-teal-950 border-teal-800 focus:ring-0"
                />
                Remember Me
              </label>
              <Link to="/forgot" className="text-amber-400/80 hover:text-amber-300 transition-all">
                Forgot Password?
              </Link>
            </div>
            ...
            <motion.button
              whileHover={!submitting ? { scale: 1.01, boxShadow: '0 0 25px rgba(245, 158, 11, 0.3)' } : {}}
              whileTap={!submitting ? { scale: 0.99 } : {}}
              type="submit"
              disabled={submitting}
              className="w-full py-3 mt-2 bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-600 text-teal-950 font-bold tracking-widest uppercase rounded-xl shadow-lg border border-amber-300/20 text-xs disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300"
            >
              {submitting ? 'ACCESSING YOUR HUB...' : 'ACCESS YOUR HUB'}
            </motion.button>

            {/* Content Split Divider Line */}
            <div className="relative flex py-1 items-center">
              <div className="flex-grow border-t border-teal-900/80"></div>
              <span className="flex-shrink mx-4 text-teal-400/30 text-[10px] tracking-widest uppercase font-mono">OR</span>
              <div className="flex-grow border-t border-teal-900/80"></div>
            </div>

            {/* Secondary Integration Action Link */}
            <button
              type="button"
              className="w-full py-2.5 bg-teal-950/40 border border-teal-800/60 text-amber-200/80 rounded-xl flex items-center justify-center gap-2 hover:bg-teal-900/40 hover:border-amber-500/30 transition-all duration-300 text-xs tracking-wider"
            >
              <span className="font-medium">Continue with Gemini</span>
            </button>

            {/* Bottom Register Redirect Link */}
            <div className="text-center text-xs text-teal-300/50 pt-1">
